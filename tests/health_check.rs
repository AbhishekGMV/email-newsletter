...
use email_newsletter::configuration::{get_config, DatabaseSettings};
...
async fn health_check_will_return_200() {
    ...
    let response = client.get(&format!("{}/health", &address.address)).send().await.expect("Failed to send request");
